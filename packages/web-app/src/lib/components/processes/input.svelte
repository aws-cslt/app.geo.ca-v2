<script lang="ts">
	...
	let { name, definition, value = $bindable() }: Props = $props();
...
{#snippet render()}
	{#if definition?.schema?.enum}
		<select id={name} {name} class={styleClass} bind:this={input} bind:value>
	...
	{:else if type == 'string'}
		{#if format == 'date-time'}
			...
			/>
		{:else if format == 'date'}
			<input class={styleClass} id={name} {name} type="date" bind:this={input} bind:value />
		{:else}
			<input class={styleClass} id={name} {name} type="text" bind:this={input} bind:value />
		{/if}
	{:else if type == 'object'}
		{#if format == 'ogc-bbox'}
